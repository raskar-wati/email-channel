import svgPaths from "./svg-97d94gcbh1";

function ContactsIconInner() {
  return (
    <div
      className="absolute bottom-[8%] left-[8%] right-[8%] top-[8%]"
      data-name="Contacts Icon Inner"
    >
      <svg
        className="block size-full"
        fill="none"
        preserveAspectRatio="none"
        viewBox="0 0 20 20"
      >
        <g id="Book Contacts">
          <path
            d="M11.5 7.5C11.5 8.32843 10.8284 9 10 9C9.17157 9 8.5 8.32843 8.5 7.5C8.5 6.67157 9.17157 6 10 6C10.8284 6 11.5 6.67157 11.5 7.5ZM7 10.75V11C7 12.1046 7.89543 13 9 13H11C12.1046 13 13 12.1046 13 11V10.75C13 10.3358 12.6642 10 12.25 10H7.75C7.33579 10 7 10.3358 7 10.75ZM4 4V16C4 17.1046 4.89543 18 6 18H15.5C15.7761 18 16 17.7761 16 17.5C16 17.2239 15.7761 17 15.5 17H6C5.44772 17 5 16.5523 5 16H15C15.5523 16 16 15.5523 16 15V4C16 2.89543 15.1046 2 14 2H6C4.89543 2 4 2.89543 4 4ZM14 3C14.5523 3 15 3.44772 15 4V15H5V4C5 3.44772 5.44772 3 6 3H14Z"
            fill="var(--fill-0, #505451)"
            id="Shape"
          />
        </g>
      </svg>
    </div>
  );
}

function FluentBookContacts24Regular() {
  return (
    <div
      className="absolute left-[-1.387px] overflow-clip size-[18px] top-[-1px]"
      data-name="fluent:book-contacts-24-regular"
    >
      <ContactsIconInner />
    </div>
  );
}

function AkarIconsInbox() {
  return (
    <div
      className="absolute left-0 overflow-clip size-[15px] top-0"
      data-name="akar-icons:inbox"
    >
      <FluentBookContacts24Regular />
    </div>
  );
}

export default function ContactsIcon() {
  return (
    <div className="relative size-full" data-name="Contacts Icon">
      <AkarIconsInbox />
    </div>
  );
}